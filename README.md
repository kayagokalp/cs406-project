...
#### Cycles vs. Circuits:
In circuit there may exist repitation of edges wheareas in a cycle there are no repitation of edges. So it should be considered that the final solution we have is for finding circuits and not cycles. So cases where edge repitions exits are also counted in the results. But, potential solutions that count cycles are also discussed and presented briefly. 
